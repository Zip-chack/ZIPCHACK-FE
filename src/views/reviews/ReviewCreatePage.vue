<template>
  <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-2xl font-bold text-gray-900 mb-2">리뷰 작성</h1>
    <p class="text-gray-600 mb-8">매물에 대한 솔직한 리뷰를 남겨주세요</p>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <!-- Residence Period -->
      <div class="card p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">거주 기간</h2>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">입주일</label>
            <input v-model="form.moveInDate" type="date" class="input" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">퇴거일</label>
            <input v-model="form.moveOutDate" type="date" class="input" />
          </div>
        </div>
      </div>
  ...
  setup() {
    ...
    async function handleSubmit() {
      try {
        ...
        await reviewAPI.createListingReview(route.params.id, reviewData)
        alert('리뷰가 등록되었습니다!')
        router.back()
      } catch (err) {
